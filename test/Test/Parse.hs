...
module Test.Parse (fileParsing, simpleParsing) where
...
fileParsing :: IO TestTree
fileParsing = do
  fixtures <- loadFixtures "test/fixtures" -- withResource (loadFixtures "test/fixtures") (const $ pure ()) $ \fixtures ->
  pure $ testParser "while file parsing" pZig fixtures []
